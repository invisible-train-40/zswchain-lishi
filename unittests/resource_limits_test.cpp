...
public:
   resource_limits_fixture()
       : chainbase_fixture(), resource_limits_manager(*chainbase_fixture::_db, []() { return nullptr; })
   {
      add_indices();
      initialize_database();
   }
...
try
{
   ...
   while (get_virtual_block_cpu_limit() > config::default_max_block_cpu_usage && iterations <= expected_relax_iterations + expected_contract_iterations)
   {
...
try
{
   ...
   // push maximum resources to go from idle back to congested as fast as possible
   while (get_virtual_block_net_limit() > config::default_max_block_net_usage && iterations <= expected_relax_iterations + expected_contract_iterations)
   {
